 ...
 *
 * sol_main.c: Solution code for main.c.
 *
 * main.c
 */
...
// Callback function for checking the PIR motion sensor
void callbackCheckMotion(uint32_t time)
{
    ...
    if (sysState == On)
    {
        ...
        if (lastDetectTime + 3000 >= time)      // Motion detected in the past 5 seconds
        {
            alarmState = On;
    ...
    // schedule the next callback
    schdCallback(callbackCheckMotion, time + 100);
}
