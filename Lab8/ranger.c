...
void    rangerInit() {
    ...
    TimerConfigure(TIMER, (TIMER_CFG_SPLIT_PAIR |TIMER_CFG_B_CAP_TIME_UP));

    TimerControlEvent(TIMER, TIMER_B, TIMER_EVENT_BOTH_EDGES);

    TimerEnable(TIMER, TIMER_B);
}

void sendStartPulse(){
    GPIOPinTypeGPIOOutput(PORTB, PIN3);
    GPIOPinWrite(PORTB,PIN3, 0);
    waitUs(2);
    GPIOPinWrite(PORTB,PIN3, PIN3);
    waitUs(5);
    GPIOPinWrite(PORTB,PIN3, 0);
}
...
uint32_t rangerDetect(){
    ...
    GPIOPinTypeTimer(PORTB, PIN3);
    GPIOPinConfigure(GPIO_PB3_T3CCP1);

    TimerIntClear(TIMER,TIMER_CAPB_EVENT);

    while(!TimerIntStatus(TIMER, false)){
    }

    timeSent = TimerValueGet(TIMER,TIMER_B);

    TimerIntClear(TIMER,TIMER_CAPB_EVENT);


    while(!TimerIntStatus(TIMER, 0)){
    }

    timeReceived = TimerValueGet(TIMER,TIMER_B);

    TimerIntClear(TIMER,TIMER_CAPA_EVENT);

    distance  = (timeReceived - timeSent)*170 / 50000;
    uprintf("distance is %d\n\r", distance);
